    ...
    def evalRPN(self, tokens: List[str]) -> int:
        ...
        for token in tokens:
            if token in OPERATORS:
                second = stack.pop()
                first = stack.pop()
                calculated_value = calculate((first, second), token)
...
def calculate(operands, operator):
    ...
    if operator == "/":
        if operands[0] < 0 or operands[1] < 0:
            return math.ceil(operands[0] / operands[1])
        return operands[0] // operands[1]


def test(tokens, expected_result):
    sol = Solution()
...
tokens = ["2","1","+","3","*"]
expected_result = 9 # ((2 + 1) * 3) = 9
print(test(tokens, expected_result))

tokens = ["4","13","5","/","+"]
expected_result = 6 # (4 + (13 / 5)) = 6
print(test(tokens, expected_result))
